...
function buildStyles() {
  ...
  return src('sass/**/*.scss')    //**=look for subfolder */

    .pipe(sass())   //to compiled into css file

    .pipe(purgecss({ content: ['*.html'] }))       //2nd arg is for watch changes in html file so it could add new class in css file accordingly

    //it will look for any public html file and see what css is using in connected css file if theres some extra css which is not being used in html then it will remove it
